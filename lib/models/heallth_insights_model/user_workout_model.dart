class UserWorkoutModel {
  int? status;
  String? errorMessage;
  List<UserWorkoutData>? data;

  UserWorkoutModel({this.status, this.errorMessage, this.data});

  UserWorkoutModel.fromJson(Map<String, dynamic> json) {
    status = json['Status'];
    errorMessage = json['ErrorMessage'];
    if (json['Data'] != null) {
      data = <UserWorkoutData>[];
      json['Data'].forEach((v) {
        data!.add(UserWorkoutData.fromJson(v));
      });
    }
  }
...
class UserWorkoutData {
  ...
  UserWorkoutData.fromJson(Map<String, dynamic> json) {
    id = json['Id'];
    userId = json['UserId'];
    points = json['Points'];
