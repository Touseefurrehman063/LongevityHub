...
import 'package:longevity_hub/screens/health_az/well_heading_detail.dart';
import 'package:shimmer/shimmer.dart';

class WellDetail extends StatefulWidget {
  ...
  @override
  _WellDetailState createState() => _WellDetailState();
}

class _WellDetailState extends State<WellDetail> {
  ...
  fetchData() async {
    await HealthController.i.fetchDetails(widget.url!);
    setState(() {
      _isLoading = false;
    });
  }
                  ...
                  children: [
                    ...
                    GetBuilder<HealthController>(builder: (cont) {
                      return ListView.builder(
                        ...
                        shrinkWrap: true,
                        physics: const NeverScrollableScrollPhysics(),
                        itemCount: cont.mainList.length,
                        itemBuilder: (context, index) {
                          var WellDetail = cont.mainList[index];
                          if (WellDetail.mainEntityOfPage.isEmpty) {
                            return const SizedBox.shrink();
                          }
                          return Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: WellDetail.mainEntityOfPage
                                .map<Widget>((entity) {
                              return Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  InkWell(
                                    onTap: () {
                                      if (entity.url != null &&
                                        ...
                                        Get.to(() => LiveWellHeadingDetail(
                                              headingname:
                                                  entity.headline ?? "",
                                              url: entity.url ?? "",
                                            ));
                                      } else {
                                        // Optionally show a message or handle the empty URL case
